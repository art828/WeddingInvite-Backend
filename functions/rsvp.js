...
function buildStatisticsText(wedding, responses) {
...
👥 Ընդհանուր հյուրերի թիվ՝ ${totalGuests}
...
async function sendStatistics(
  ...
  const text = buildStatisticsText(wedding, responses);
...
async function sendWelcomeMessage(env, chatId) {
  const text = `
💍 Բարի գալուստ Wedding Invite Bot

Այս bot-ի միջոցով կարող եք հետևել ձեր հարսանեկան հրավիրատոմսի RSVP պատասխաններին։
...
Միացնելուց հետո կարող եք տեսնել վիճակագրությունը, հյուրերի ցանկը, հարսի և փեսայի կողմերի հյուրերին և չգալու պատասխանները։
`.trim();

  await sendTelegramMessage(env, chatId, text);
}

async function sendHelpMessage(env, chatId, wedding) {
  const text = `
...
📊 Վիճակագրություն
Ցույց է տալիս ընդհանուր պատասխանների և հյուրերի քանակը։

📋 Հյուրերի ցանկ
Ցույց է տալիս բոլոր RSVP պատասխանները։

👰 Հարսի կողմ
Ցույց է տալիս հարսի կողմից եկող հյուրերին։

🤵 Փեսայի կողմ
Ցույց է տալիս փեսայի կողմից եկող հյուրերին։

❌ Չեն գալու
Ցույց է տալիս այն հյուրերին, որոնք չեն կարող գալ։

🔄 Թարմացնել
Կրկին ցույց է տալիս վերջին վիճակագրությունը։

Կարող եք նաև օգտագործել հրամանները՝
/stats
/list
/bride
/groom
...
async function sendConnectedWelcome(
  ...
  const statsText = buildStatisticsText(
    wedding,
    responses
  );
...
export function onRequestGet() {
  return jsonResponse({
    success: true,
    message: "Telegram webhook v3 is working"
  });
}